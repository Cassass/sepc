...
int main(int argc, char **argv)
{
   ...
   *********************/
    ...
    int fd = shm_open("/tampon", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    ftruncate(fd, 4*TAILLE_X * TAILLE_Y);
    void* tampon = mmap(NULL, 4*TAILLE_X * TAILLE_Y, 
  ...
  fin:
    ...
    shm_unlink("/tampon");
    munmap(tampon, 4*TAILLE_X * TAILLE_Y);
    SDL_Quit();
